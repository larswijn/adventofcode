...
directions1 = [[x.strip()[0], int(x.strip()[1:])] for x in inputs[0].split(',')]
directions2 = [[x.strip()[0], int(x.strip()[1:])] for x in inputs[1].split(',')]

path1 = [(0, 0), ]
path2 = [(0, 0), ]
print("initialized")
start_time = time.time()
for path, directions in ((path1, directions1), (path2, directions2)):
    current_spot = list(path[-1])
    for direction in directions:
        direction[0] = direction[0].upper()
        if direction[0] == 'U':
            path.extend([(current_spot[0]+i, current_spot[1]) for i in range(1, direction[1]+1)])
            current_spot[0] += direction[1]
        elif direction[0] == 'D':
            path.extend([(current_spot[0]-i, current_spot[1]) for i in range(1, direction[1]+1)])
            current_spot[0] -= direction[1]
        elif direction[0] == 'L':
            path.extend([(current_spot[0], current_spot[1]-i) for i in range(1, direction[1]+1)])
            current_spot[1] -= direction[1]
        elif direction[0] == 'R':
            path.extend([(current_spot[0], current_spot[1]+i) for i in range(1, direction[1]+1)])
            current_spot[1] += direction[1]
        else:
...
path1_set, path2_set = set(path1), set(path2)
intersections = path1_set.intersection(path2_set)
print("generated intersections")
print(intersections)
...
earliest = min(intersections, key=lambda x: path1.index(x) + path2.index(x))
print("earliest:", path1.index(earliest) + path2.index(earliest))
print(f" ({round(time.time()-start_time, 5)} seconds)")
