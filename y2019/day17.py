...
def pt1():
    ...
    for x in out:
        ...
        view += chr(x)
        if x == 35:
            tiles.add((view.count('\n')-2,
                       len(view.rsplit('\n', 1)[-1])-1,
                       ))
    print(view)
    print(list(sorted(list(tiles))))
    intersections = 0
    alignment_sum = 0
    for tile in sorted(list(tiles)):
        if ((tile[0], tile[1]+1) in tiles and (tile[0], tile[1]-1) in tiles and (tile[0]+1, tile[1]) in tiles and (tile[0]-1, tile[1]) in tiles):
            print("intersection at", tile, " aligment:", tile[0]*tile[1])
            intersections += 1
            alignment_sum += tile[0]*tile[1]
    print("intersections:", intersections)
    print("alignment_sum:", alignment_sum)
    return {'view': view, 'intersections': intersections}


def conv_movement(movement: str):
    # for pt 2
	inputs = []
	for item in movement.strip().replace('\n', ',\n,').replace(',,', ',').split(','):
		if len(item.strip(' ')) == 0:
			continue
		if item.isdigit():
			inputs.append(int(item.strip()))
		elif item.strip(' ') == '\n':
			if inputs[-1] == ',':
				inputs.pop()
			inputs.append('\n')
			...
			inputs.extend([item.strip()[0], ',', int(item.strip()[1:])])
			continue
		inputs.append(',')
	while inputs[-1] in {'\n', ','}:
		inputs.pop()
	return [ord(str(x)) for x in inputs] + [ord('\n')]
...
START
R10,L8,R10,R4,(A,)L6,L6,R10,R10,(B,)L8,R10,R4,L6,R12,R12,(C,)R10,L6,L6,R10,(D,)L6,R12,R12,R10,L10,(E,),
    L8,R10,R4,(F,)L6,L6,R10,R10,(G,)L8,R10,R4,L6,R12,R12,(H,)R10
->
R5,5,L8,R5,5,R4,(A,)L6,L6,R5,5,R5,5,(B,)L8,R5,5,R4,L6,R6,6,R6,6,(C,)R5,5,L6,L6,R5,5,(D,)L6,R5,5,2,R5,5,2,R5,5,L5,5,(E,),
     L8,R5,5,R4,(F,)L6,L6,R5,5,R5,5,(G,)
     L8,R5,5,R4,    L6,R6,6,R6,6,(H,)R5,5
->
R5,5,L8,R5,5,R4,L6,L6,R5,5,R5,5,L8,R5,5,R4,L6,R6,6,R6,6,R5,5,L6,L6,R5,5,L6,R6,6,R6,6,R5,5,L5,5,L8,R5,5,R4,L6,L6,R5,5,R5,5,L8,R5,5,R4,L6,R6,6,R6,6,R5,5
A,B,C,A,A,
A: R5,5  # 4/20 ch
B: 5,5,L8,R5,5,R4,L6  # 17/20 ch
C: R6,6,R6,6,R5,5  # 14/20 ch
'''
